// LayerForge v9 Appendix — English version with figures/ folder reference
...
children.push(Body(
  "Complete verification record list (46 entries: Table B1 V-001-V-020 = 20 rows, Table B2 V-021-V-042-quad + 1 cross-corpus = 26 rows). Each row shows V-ID, dataset, N, verdict, and " +
  "commit hash for traceability. Full Sections 1-8 records are in docs/verifications/*.md."
));
...
children.push(Body(
  ...
  "Baseline snapshot v1 is preserved unchanged as a freeze decision under bounded ablation budget — " +
  "no parameter change reached the IMPROVEMENT-LARGE threshold (>0.30 delta) under the tested N (5-30). " +
  "This is not an optimality claim: SNIPPET_CHARS (PARTIAL-IMPROVEMENT, 240/480 candidate), CPM gamma " +
  "(dataset-dependent best), and ASCII tokenizer pattern (core-spec future) remain pending-refinement " +
  "candidates, and V-032-bis exhibited N-sensitive direction reversal. See main Section 4.5."
));
...
children.push(tblC);
children.push(Caption("Table C. Parameter ablation full details (16 entries: 14 ablation + 2 articulation correction). No parameter change reached the IMPROVEMENT-LARGE threshold (>0.30 delta) under tested N (5-30); baseline snapshot v1 is frozen under bounded ablation budget (not an optimality claim; see main Section 4.5)."));
...
children.push(Body(
  ...
  "vs fact-level fidelity) visualizes the quintuple-FAIL region across all measured points."
));

[
  ['v040_plot1_reduction_vs_theme_fidelity_en.png', 540, 378, "Plot 1. Reduction vs theme-level fidelity Pareto frontier (= main paper Figure 3)."],
  ['v040_plot2_reduction_vs_fact_fidelity_en.png', 540, 378, "Plot 2. Reduction vs fact-level fidelity. All measured points cluster in PRESERVE-FAIL region (<=0.30 threshold), visualizing the constitutive fact-level FAIL limit (see Section 2 disclaimer: metric-multiple but corpus-dual evidence)."],
  ['v040_plot3_theme_vs_fact_en.png', 540, 378, "Plot 3. Theme vs fact two-layer structure (= main paper Figure 1). LayerForge variants in lower-right quadrant (theme PASS / fact FAIL)."],
  ['v040_plot4_reduction_landscape_en.png', 540, 315, "Plot 4. Reduction landscape across 14 datasets (= main paper Figure 2). Colored by language and tokenization."],
  ['v040_plot5_cost_vs_accuracy_en.png', 540, 378, "Plot 5. Per-query input-token cost vs downstream LLM accuracy (= main paper Figure 4; see main Figure 4 caption for full details). Raw data: v029a_cost_latency_results.json."],
].forEach(([fn, w, h, cap]) => {
  embedImage(path.join(FIG_DIR, fn), w, h, cap).forEach(p => children.push(p));
});
